...
function onDragLeave(evt) {
     ...
     * events back to back. Instead, we use a 25px buffer around the window frame
     ...
     * enough acceleration).
     */
    if(evt.pageX < 25 || evt.pageY < 25 || $(window).width() - evt.pageX < 25  || $(window).height - evt.pageY < 25) {
        $("#drop-box-overlay").hide();
        $("#drop-box-prompt").hide();
    }
}
...
function onDrop(evt) {
    ...
    // Only continue with Chrome and Firefox
    var isChrome = navigator.userAgent.toLowerCase().indexOf('chrome') > -1;
    var isFirefox = navigator.userAgent.toLowerCase().indexOf('firefox') > -1;
    if(!isChrome && !isFirefox) {
        return;
    ...
    for(var i = 0, length = files.length; i < length; i++) {
        queueFile(files[i], length);
    }
}

function uploadFile(file, totalFiles, entity_key, description) {
    ...
    reader.onloadend = function(evt) {
        ...
        if(data.length > 128){
            ...
            if(base64StartIndex < data.length) {
                ...
                $.ajax({
                    type: 'POST',
                    url: '/attachments/upload?entity_key=' + entity_key,
...
function uploadNext() {
    ...
    $('#queue li:first').each(function(index, node) {
        // This will effectively upload all files at once
        uploadFile($(this).data('file'), $('queue.li').length, $(this).data('entity_key'), $('#' + $(this).data('descriptionId')).val());
        $(this).remove();
    });
}

$(function() {
    $('#queue-upload').click(function(event) {
        $(this).hide();
    ...
    $('#queue-cancel').live('click', function(event) {
        $('.queue-remove li').remove();
        $('#queue-container').hide();
    });
